...
#         log_file = os.path.abspath('resources/journalInstallation.txt')
...
#         QTimer.singleShot(000, self.close)  # 2000 ms = 2 secondes
...
class LogWorker:
    def __init__(self):
        ...
        self.log_file = os.path.abspath('resources/journalInstallation.txt')

        self.start_logging()

    def start_logging(self):
        """Démarre le processus de gestion des dossiers et journalise les événements"""
        current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

        # Journaliser le démarrage du script
        print(f"[INFO] - Script démarré le {current_time}\n")
        self.log_to_file(f"[INFO] - Script démarré le {current_time}\n")

        # Vérifier et créer le dossier "bon"
        if not os.path.exists(self.user_folder):
            os.makedirs(self.user_folder)
            print(f"[INFO] - Le dossier 'bon' a été créé avec succès.\n")
            self.log_to_file(f"[INFO] - Dossier 'bon' créé avec succès le {current_time}\n")
        else:
            print(f"[INFO] - Le dossier 'bon' existe déjà.\n")
            self.log_to_file(f"[INFO] - Dossier 'bon' existant vérifié le {current_time}\n")

        # Vérifier et créer le dossier "AI-FB-Robot" dans "Downloads"
        if not os.path.exists(self.download_folder):
            os.makedirs(self.download_folder)
            print(f"[INFO] - Le dossier 'AI-FB-Robot' a été créé avec succès.\n")
            self.log_to_file(f"[INFO] - Dossier 'AI-FB-Robot' créé avec succès le {current_time}\n")
        else:
            print(f"[INFO] - Le dossier 'AI-FB-Robot' existe déjà.\n")
            self.log_to_file(f"[INFO] - Dossier 'AI-FB-Robot' existant vérifié le {current_time}\n")

        # Fin du script
        print(f"[INFO] - Script terminé le {current_time}\n")
        self.log_to_file(f"[INFO] - Script terminé le {current_time}\n")
...
def main():
    # Démarre le processus de gestion des dossiers et journalisation
    LogWorker()

    # Pause de 2 secondes avant de terminer le programme (simule la fermeture automatique)
    time.sleep(2)
