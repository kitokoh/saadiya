...
#         QTimer.singleShot(0, self.close)  # Ferme l'application après 2 secondes
...
# from datetime import datetime
# from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QLabel, QMessageBox
# from PyQt5.QtCore import QTimer  # Importer QTimer
...
#         # Utilisation d'un timer pour fermer l'application après 2 secondes
#         QTimer.singleShot(2000, self.close)  # Ferme l'application après 2 secondes
...
class LicenseGenerator:
    ...
    def generate_license(self):
        ...
        if self.error_flag == 0:
            ...
            self.log(f"[ERROR] - Le script s'est terminé avec des erreurs le {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
            print("Des erreurs se sont produites lors de la génération de la licence.")
