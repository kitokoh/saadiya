...
def copy_file():
    """Copier le fichier cle.exe du répertoire courant vers le Bureau de l'utilisateur."""
    desktop_path = os.path.join(os.path.expanduser("~"), "Desktop")
    source_folder = os.path.join(os.getcwd(), "resources", "tools")  # Dossier source modifié
    source_file = os.path.join(source_folder, "cle.exe")
    dest_file = os.path.join(desktop_path, "FB-Robot.exe")
    ...
    if os.path.exists(source_file):
        try:
            ...
            if os.path.exists(dest_file):
                print("Succès : La copie de cle.exe vers le Bureau a réussi !")
            else:
                print("Échec : Le fichier n'a pas été trouvé sur le Bureau.")
        except Exception as e:
            print(f"Erreur lors de la copie : {str(e)}")
    else:
        print("Erreur : Le fichier source cle.exe n'a pas été trouvé.")
