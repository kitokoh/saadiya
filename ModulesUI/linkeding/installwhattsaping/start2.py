...
#     # Chemins d'accès
#     env_script_path = f"C:\\bon\\robot{instance_number}\\env{instance_number}\\Scripts"
#     robot_path = f"C:\\bon\\robot{instance_number}"
...
def main():
    ...
    for entry in os.scandir(base_path):
        if entry.is_dir() and entry.name.startswith("robot"):
            ...
            try:
                ...
                threads.append(thread)
                thread.start()  # Démarrer l'exécution de chaque instance
                print(f"Exécution de l'instance robot{instance_number} démarrée.")
            except ValueError:
                print(f"Nom de dossier non valide : {entry.name}")
    ...
    print("Toutes les instances de robot ont été exécutées.")
