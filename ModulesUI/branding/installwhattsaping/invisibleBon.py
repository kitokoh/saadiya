...
class FolderVisibilityApp(QWidget):
    ...
    def initUI(self):
        self.setWindowTitle("Gestion de la Visibilité des Dossiers")

        layout = QVBoxLayout()

        self.label = QLabel("Entrez le chemin du dossier à rendre invisible :")
        layout.addWidget(self.label)
        ...
        self.check_button = QPushButton("Vérifier et Rendre Invisible", self)
        self.check_button.clicked.connect(self.check_and_hide_folder)
        layout.addWidget(self.check_button)
    ...
    def check_and_hide_folder(self):
        ...
        if os.path.exists(folder_path):
            # Commande pour rendre le dossier caché et système
            os.system(f'attrib +h +s "{folder_path}"')
            QMessageBox.information(self, "Succès", f"Le dossier '{folder_path}' est maintenant invisible.")
        else:
            QMessageBox.critical(self, "Erreur", f"Le dossier '{folder_path}' n'existe pas.")
