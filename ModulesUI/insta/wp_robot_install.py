...
# Option spécifique pour masquer la console sous Windows (Win32 et Win64)
                    ...
                    #self.face_main = WpRobot()  # Remplacez ceci par votre classe de page d'accueil
                    #self.face_main.show()
...
# Initialisation de la langue après la création des actions
    def switch_language(self, language):
        """Permet de changer la langue."""
        if language == "en":
            self.translator.load(os.path.join(user_data_dir, 'resources', 'settings.json'))
        elif language == "fr":
            self.translator.load(os.path.join(user_data_dir, 'resources', 'lang','en_US','whatssaping','fb_robot_install_translated.qm'))
        elif language == "tr":
            self.translator.load(os.path.join(user_data_dir, 'resources', 'lang','en_US','whatssaping','fb_robot_install_translated.qm'))
...
class WpRobot(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        # Ajouter le header
        header = HeaderSection(self, title="Insta Robot Install Instances", app_name="Insta ROBOT AI", slogan="AI Marketing Automation Solutions")
...
# Ajouter les boutons conditionnels
  ...
  # Utilisation de self.tr()
    ...
    def retranslateUi(self):
        """Recharge les textes traduits dans l'interface."""
        self.setWindowTitle(self.tr('AI Insta ROBOT Pro'))
        # Mettez à jour ici tous les labels, boutons, menus, etc. avec self.tr()
        # Par exemple, pour les actions du menu :
