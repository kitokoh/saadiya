...
def show_preview(self, media):
    dialog = PreviewDialog(media, self)
    self.media_table_widget.itemDoubleClicked.connect(lambda item: self.show_preview(item.media_data))
...
class AddMedia(QWidget):
    ...
    def add_media(self):
        ...
        # Autoriser la sélection de plusieurs fichiers média
        files, _ = QFileDialog.getOpenFileNames(
            self, 
            "Sélectionnez des fichiers média", 
            "", 
        ...
        if files:
            ...
            # Récupérer le plus grand numéro de clé existant
            for media in media_data['posts']:
                key = int(media['image'].split('\\')[-1].split('.')[0])  # Numéro d'image extrait du chemin
                if key > max_key:
                    max_key = key
            
            # Boucler sur chaque fichier sélectionné
            for file_path in files:
                extension = os.path.splitext(file_path)[1]  # Récupérer l'extension du fichier
                max_key += 1  # Incrémenter la clé pour chaque fichier
                new_name = f"{max_key}{extension}"  # Renommer en utilisant la nouvelle clé

                # Définir le chemin de destination dans le dossier média
                ...
                # Copier le fichier dans le dossier média avec un nouveau nom
                shutil.copy(file_path, new_path)
                
                # Ajouter les informations du nouveau média dans le JSON
                media_data['posts'].append({
                    "text": "Description du média",  # Description par défaut, modifiable plus tard
                    "image": new_path
                })

            # Mettre à jour le fichier JSON avec les nouvelles données
            self.update_json(media_data)
            QMessageBox.information(self, self.tr("Succès"),  "Média ajouté avec succès !")

        # Exemple de création de la fenêtre depuis votre classe principale
    # Assurez-vous que 'self' est une instance de QWidget (comme MainWindow par exemple)
    #add_media_window = AddMedia(parent=self)
...
class MediaTable(QWidget):
    def __init__(self, parent=None):
        ...
        # Ajouter le header
        header = HeaderSection(self, title="self.tr('AI Medias Mangements')", app_name="Nova360 AI", slogan="AI Marketing & Management Auto")
        ...
        self.update_json_button = QPushButton('Mettre à jour JSON', self)
...
# Lancer le transfert de fichiers
    def get_instance_list(self):
        ...
        return [f'Instance {i+1}' for i in range(len(instance_folders))]
...
# Intégrer le bouton "Aperçu" dans votre classe principale
    ...
    def show_preview(self, media_path):
        description = "Votre description ici"  # Récupérez la description réelle du média
    ...
    def create_action_menu(self, media):
        ...
        delete_action = menu.addAction('Appercu')
        delete_action.triggered.connect(lambda: self.update_media(media))
        return menu
    ...
    def delete_media(self, media):
        msg_box = QMessageBox.question(self, self.tr('Confirmation'),  f'Êtes-vous sûr de vouloir supprimer {media["nom"]} ?', QMessageBox.Yes | QMessageBox.No)
    ...
    def switch_language(self, language):
        """Permet de changer la langue."""
        if language == "en":
            self.translator.load("lang/en_US/modules/media_manager.qm")
        elif language == "fr":
            self.translator.load("lang/fr_FR/modules/media_manager.qm")
        elif language == "tr":
            self.translator.load("lang/tr_TR/modules/media_manager.qm")
        elif language == "ar":
            self.translator.load("lang/ar_AR/modules/media_manager.qm")
        QApplication.instance().installTranslator(self.translator)
        self.retranslateUi()
    ...
    def update_json(self, data, file_path='data/media.json'):
            ...
            try:
                ...
                print(f"Les données ont été mises à jour dans {file_path}.")
